...
from pattern import StmtInsPos
...
class Pipeline:
    ...
    def load_and_convert(self):
        ...
        with gzip.open(os.path.join(self.root, "oj.pkl.gz"), 'rb') as f:
            d = pickle.load(f)
        valid_ratio = 0.2
        train_raw = []
        test_raw = []

        train_len = len(d['raw_tr'])
        test_len = len(d['raw_te'])
        ...
        train_idx = []
        train_err_cnt = 0
        for i in tqdm.tqdm(range(train_len)):
            try:
                train_raw_ = parser.parse(train_raw[i])
                train_idx.append(i)
                train_ast.append(train_raw_)
                train_label.append(d['y_tr'][i])
            except:
                train_err_cnt += 1
#                 print(i)
#                 print(train['raw'][i])
...
#                 exit()
        ...
        train_dict = {'0': train_idx, '1': train_ast, '2': train_label}
        ...
        test_idx = []
        test_err_cnt = 0
        for i in tqdm.tqdm(range(test_len)):
            try:
                test_raw_ = parser.parse(test_raw[i])
                test_idx.append(i)
                test_ast.append(test_raw_)
                test_label.append(d['y_te'][i])
            except:
                test_err_cnt += 1
            
        print('test data parsing error number: ', test_err_cnt)
    
        test_dict = {'0': test_idx, '1': test_ast, '2': test_label}
