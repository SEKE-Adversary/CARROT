...
class TBCNNClassifier(torch.nn.Module):
    def __init__(self,
        ...
        self.classify=nn.Linear(h_size,n_classes)
        self.pooling=GlobalAttentionPooling(nn.Linear(h_size,1))
        #self.pooling=AvgPooling()

    def _forward(self, batch,root_ids=None):
    ...
    def forward(self, batch1, batch2):
        ...
        vec1 = self._forward(batch1)
        vec2 = self._forward(batch2)
    ...
    def prob(self, batch1, batch2):
        logits = self.forward(batch1, batch2)
        prob = nn.Softmax(dim=-1)(logits)
        return prob

    def grad(self, batch1, batch2, labels, loss_fn):

        vec2 = self._forward(batch2)
        vec2_const = vec2.clone().detach() #torch.tensor(vec2, requires_grad=False)
