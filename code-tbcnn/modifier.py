...
def get_batched_data(raws, ys, node2idx, vocab_size, ids=None):

    nodes, begins, ends = raw2node(raws, node2idx, vocab_size)
    graphs = [dgl.graph((torch.tensor(b), torch.tensor(e))) for b,e in zip(begins, ends)]
    for graph, node in zip(graphs, nodes):
        graph.ndata['type'] = torch.tensor(node)
    # raws, nodes, begins, ends, graphs, ys, ids
    
    batch = {"graph": [], "y": [], "raw": [], "id": [], "new_epoch": False}
    batch['graph'] = dgl.batch(graphs)
    batch['y'] = ys
    batch['id'] = ids
...
class TokenModifier(object):
    ...
    def rename_uid(self, x_raw, y, ori_uid, n_candidate=5):
        ...
        grad = self.cl.grad(inputs, labels, self.loss)
        grad = grad[ori_uid].reshape([1, self.cl.x_size])
        ori_embed = self.cl.embeddings.weight[ori_uid]\
                    .reshape([1, self.cl.x_size])\
                    .expand([self.cl.vocab_size, self.cl.x_size])
        delta_embed = self.uids * (self.cl.embeddings.weight - ori_embed)
