# -*- coding: utf-8 -*-
"""
Created on Mon Mar 30 11:13:02 2020
...
class BatchProgramCC(nn.Module):
    ...
    def grad(self, x1, x2, y, loss_fn):
        
        rvec = self.encode(x2)
        rvec_const = Variable(rvec, requires_grad=False)
        
        self.zero_grad()
        lvec = self.encode(x1)
        abs_dist = torch.abs(torch.add(lvec, -rvec_const))
        pred = torch.sigmoid(self.hidden2label(abs_dist))
        loss = loss_fn(pred, y)
        #print (rvec.requires_grad)
        #print (rvec_const.requires_grad)
        loss.backward()
