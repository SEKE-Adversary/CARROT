...
class Attacker(object):
    ...
    def attack(self, x_raw, x2_raw, y, uids, n_candidate=100, n_iter=20, relax=1):
        ...
        if torch.argmax(old_prob) != y:
            ...
            return True, x_raw, x2_raw, torch.argmax(old_prob).cpu().numpy()
        old_prob = old_prob[y]
        while iter < n_iter:
            keys = list(uids.keys())
            for k in keys:
                ...
                if new_prob[new_prob_idx][y] * relax < old_prob:
                    ...
                    print ("rej\t%s" % k)
        print ("FAIL!")
        return False, ori_x_raw, x2_raw, y
    ...
    def attack_all(self, n_candidate=100, n_iter=20, relax=1, res_save=None):
        ...
        for i in range(self.d.test.get_size()):
            ...
            tag, x, x2, pred = self.attack(b['raw1'][0], b['raw2'][0], b['y'][0],
                                               self.syms['te'][b['id1'][0]], n_candidate, n_iter, relax)
            #except Exception as e:
            #    tag, x, x2, pred = False, b['raw1'][0], b['raw2'][0], b['y'][0]
            #    print ("FATAL ERROR! SHUTDOWN!", end='\n\t')
            #    print (e)
            if tag:
                n_succ += 1
        ...
        if res_save is not None:
            with gzip.open(res_save, "wb") as f:
                pickle.dump({"raw1": raw1s, "raw2": raw2s, "y": trues}, f)
        ...
        while iter < n_iter:
            ...
            for insD, p, pr, _x_raw in zip(new_insertDict, new_pred, new_prob, new_x_raw):
                if p != y:
                    ...
                    return True, _x_raw, x2_raw, p.cpu().numpy()
    ...
    def attack_all(self, n_candidate=100, n_iter=20, relax=1, res_save=None):
        ...
        for i in range(self.d.test.get_size()):
            ...
            print ("\t%d/%d\tID = (%d, %d)\tY = %d" %
                   (i+1, self.d.test.get_size(), b['id1'][0], b['id2'][0], b['y'][0]))
            tag, x, x2, pred = self.attack(b['raw1'][0], b['raw2'][0], b['y'][0], 
                                           self.inss['stmt_te'][b['id1'][0]], n_candidate, n_iter, relax)
            if tag:
                n_succ += 1
                total_time += time.time() - start_time
            # print (pred)
...
class AttackerRandom(object):
    ...
    def attack(self, x_raw, x2_raw, y, uids, n_iter=20, relax=1):
        ...
        while iter < n_iter:
            ...
            for k in keys:
                ...
                if k in self.tokenM.forbidden_uid:
                    ...
                    continue
                iter += 1
                new_x_raw, new_x_ast, new_x_uid = self.tokenM.rename_uid_random(x_raw, k)
                if new_x_raw is None:
                    n_stop += 1
                    print ("skip unk\t%s" % k)
                    continue
    ...
    def attack_all(self, n_iter=20, relax=1, res_save=None):
        ...
        for i in range(self.d.test.get_size()):
            ...
            start_time = time.time()
            tag, x, x2, pred = self.attack(b['raw1'][0], b['raw2'][0], b['y'][0],
                                               self.syms['te'][b['id1'][0]], n_iter, relax)
            #except Exception as e:
            #    tag, x, x2, pred = False, b['raw1'][0], b['raw2'][0], b['y'][0]
            #    print ("FATAL ERROR! SHUTDOWN!", end='\n\t')
            #    print (e)
...
class InsAttackerRandom(object):
    ...
    # only support single x: a token-idx list
    def attack(self, x_raw, x2_raw, y, poses, n_iter=20, relax=1):
        ...
        while iter < n_iter:
            ...
            # get insertion candidates
            new_x_raw, new_x_ast, new_x2_ast, new_insertDict = self.insM.insert_remove_random(x_raw, x2_raw)
            if new_x_raw == []: # no valid candidates
                n_stop += 1
                continue
    ...
    def attack_all(self, n_iter=20, relax=1, res_save=None):
        ...
        for i in range(self.d.test.get_size()):
            ...
            print ("\t%d/%d\tID = (%d, %d)\tY = %d" %
                   (i+1, self.d.test.get_size(), b['id1'][0], b['id2'][0], b['y'][0]))
            tag, x, x2, pred = self.attack(b['raw1'][0], b['raw2'][0], b['y'][0], 
                                           self.inss['stmt_te'][b['id1'][0]], n_iter, relax)
            if tag:
                n_succ += 1
                total_time += time.time() - start_time
            # print (pred)
...
if __name__ == "__main__":
    ...
    #model.load_state_dict(torch.load('../model_clone/astnn_uid_rename_adv/model.pt'))
    #model.load_state_dict(torch.load('../model_clone/astnn_stmt_insert_adv/model.pt'))
    #model.load_state_dict(torch.load('../model_clone/astnn_cross_adv/model.pt'))
    model.load_state_dict(torch.load('../model_clone/mhm_astnn_model.pt'))
    #=========================== attack after adv-training =========================
    
    #atk = Attacker(oj, symtab, model, word2vec)
    #atk.attack_all(40, 50, 1)

    #atk = InsAttacker(oj, instab, model, word2vec)
    #atk.attack_all(40, 20, 1)

    #atk = AttackerRandom(oj, symtab, model, word2vec)
    #atk.attack_all(100)

    atk = InsAttackerRandom(oj, instab, model, word2vec)
    atk.attack_all(20)