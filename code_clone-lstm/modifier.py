...
#sys.path.append("E:\\CARROT\\Universal-MHM\\preprocess-lstm")
import pattern
import random
...
class TokenModifier(object):
    ...
    def rename_uid(self, x, x2, y, x_uid, ori_uid, n_candidate=5):
        ...
        _x2 = torch.tensor(x2, dtype=torch.long).cuda().permute([1, 0])
        _y = torch.tensor(y, dtype=torch.long).cuda()
        grad, grad2 = self.cl.grad(_x, _x2, _y, self.loss)
        grad = grad[0]
...
class InsModifier(object):
    ...
    def insert(self, x, x2, n_candidate=5):
        ...
        for pos in pos_candidates:
            ...
            new_x.append(_x)
            new_x2.append(deepcopy(x2))
            new_insertDict.append(_insertDict)

        return new_x, new_x2, new_insertDict

    # only support one piece of data each time: x is idx-list
    def remove(self, x, x2, n_candidate=5):
    ...
    def insert_remove_random(self, x, x2):
        ...
        while True:
            ...
            _x = pattern.InsResult(x, _insertDict)
            new_x.append(_x)
            new_x2.append(copy.deepcopy(x2))
            new_insertDict.append(_insertDict)
            break
        return new_x, new_x2, new_insertDict
...
if __name__ == "__main__":
    ...
    x = torch.tensor(b['x1'], dtype=torch.long).cuda().permute([1, 0])
    x2 = torch.tensor(b['x2'], dtype=torch.long).cuda().permute([1, 0])
    y = torch.tensor(b['y'], dtype=torch.long).cuda()
    print (b['y'][0])
    prob = classifier.prob(x, x2)[0]
    print (int(torch.argmax(prob)), float(prob[b['y'][0]]))
    
    old_x, old_x2 = b['x1'][0], b['x2'][0]
    for _ in range(3):
        new_x, new_x2, new_insertDict = m.insert(old_x, old_x2, n_candidate=3)
        feed_new_x = [_x[:classifier.max_len] for _x in new_x]  # this step is very important
        new_prob = classifier.prob(
    ...
    for _ in range(2):
        new_x, new_x2, new_insertDict = m.remove(old_x, old_x2, n_candidate=3)
        feed_new_x = [_x[:classifier.max_len] for _x in new_x]  # this step is very important
        new_prob = classifier.prob(
          torch.tensor(feed_new_x, dtype=torch.long).cuda().permute([1, 0]),
          torch.tensor(new_x2, dtype=torch.long).cuda().permute([1, 0]))
