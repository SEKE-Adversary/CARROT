...
class Dataset(object):
    ...
    def __init__(self, xs=[], ys=[], seqs=[], raws=None, ids=None, classes=None,
        ...
        assert len(self.__idx2txt) == self.__vocab_size \
            ...
            "len(self.__idx2txt)=%d len(self.__txt2idx)=%d __vocab_size=%d" % (len(self.__idx2txt), len(self.__txt2idx), self.__vocab_size)
...
class OJ104(object):
    ...
    def __init__(self, path='../data_clone/oj.pkl.gz', max_len=500, vocab_size=2000,
        ...
        tr_raws = d['tr']['raw']
        tr_classes = d['tr']['y']
        if adv_train_path != None:  # add adversarial samples for training
            tr_raws = None
            tr_classes = None
            with gzip.open(adv_train_path, "rb") as f:
                tmp_d = pickle.load(f)
                adv_x1 = tmp_d["adv_x1s"]
                adv_x2 = tmp_d["adv_x2s"]
                adv_y = tmp_d["adv_labels"]
            if adv_train_size != None:
                tmp_idxs = random.sample(range(len(adv_x1)), adv_train_size)
                adv_x1_ = [adv_x1[i] for i in tmp_idxs]
                adv_x2_ = [adv_x2[i] for i in tmp_idxs]
                adv_y_ = [adv_y[i] for i in tmp_idxs]
                adv_x1, adv_x2, adv_y = adv_x1_, adv_x2_, adv_y_
            # token idx 0 used as padding.
            adv_x1 = [remove_tail_padding(x, 0) for x in adv_x1]
            adv_x2 = [remove_tail_padding(x, 0) for x in adv_x2]
            for _x1, _x2, _y in zip(adv_x1, adv_x2, adv_y):
                len_tr_seqs = len(tr_seqs)
                x.append((len_tr_seqs, len_tr_seqs+1))
                tr_seqs.append(_x1)
                tr_seqs.append(_x2)
                y.append(_y)
                #print(x[-1], y[-1], len(tr_seqs))
            print("[Adversarial Training] adversarial sample number: %d" % len(adv_x1), flush=True)
