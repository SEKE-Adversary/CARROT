...
class OJ104(object):
    ...
    def __init__(self, path='../data/oj.pkl.gz', max_len=500, vocab_size=5000,
        ...
        for i, t in zip(range(vocab_size), self.__idx2txt): # "<unk>" is "<pad>"
            self.__txt2idx[t] = i
        ...
        if adv_train_path != None:  # add adversarial samples for training
            with gzip.open(adv_train_path, "rb") as f:
                tmp_d = pickle.load(f)
                adv_raw = tmp_d["adv_raw"]
                adv_label = tmp_d["adv_label"]
            if adv_train_size != None:
                tmp_idxs = random.sample(range(len(adv_raw)), adv_train_size)
                adv_raw_ = [adv_raw[i] for i in tmp_idxs]
                adv_label_ = [adv_label[i] for i in tmp_idxs]
                adv_raw, adv_label = adv_raw_, adv_label_
            for _raw, _label in zip(adv_raw, adv_label):
                ids.append(ids[-1]+1)
                raw.append(_raw)
                x.append(None)    # ASTNN training need raw rather than x
                y.append(_label)
