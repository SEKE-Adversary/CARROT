...
class AdversarialTrainingAttacker(object):
    ...
    def attack_all(self, n_candidate=100, n_iter=20, res_save=None, adv_sample_size=5000):
        ...
        total_time = 0
        adv_raws, adv_labels = [], []
        fail_pred_raws, fail_pred_labels = [], []
        st_time = time.time()
        for i in range(self.d.train.get_size()):
            if len(adv_raws) >= adv_sample_size:
                break
            b = self.d.train.next_batch(1)
            print ("\t%d/%d\tID = %d\tY = %d" % (i+1, self.d.train.get_size(), b['id'][0], b['y'][0]))
            start_time = time.time()
            tag, x, typ =  self.attack(b['raw'][0], b['y'][0], self.syms['tr'][b['id'][0]], n_candidate, n_iter)
            if tag:
                n_succ += 1
                total_time += time.time() - start_time
                fail_pred_raws.append(x)
        ...
        if res_save is not None:
            print ("Adversarial Sample Number: %d (Out of %d False Predicted Sample)" % (len(adv_raws), len(fail_pred_raws)))
            with gzip.open(res_save, "wb") as f:
                pickle.dump({"fail_pred_raw": fail_pred_raws, 
                             "fail_pred_label": fail_pred_labels,
                             "adv_raw": adv_raws, 
                             "adv_label": adv_labels}, f)
...
class AdversarialTrainingInsAttacker(object):
    ...
    def attack(self, x_raw, y, poses, n_candidate=100, n_iter=20):
        ...
        while iter < n_iter:
            ...
            new_prob = self.cl.prob(x_in)
            new_pred = torch.argmax(new_prob, dim=1)
            for insD, p, pr, _raw in zip(new_insertDict, new_pred, new_prob, new_x_raw):
                if p != y:
    ...
    def attack_all(self, n_candidate=100, n_iter=20, res_save=None, adv_sample_size=5000):
        ...
        for i in range(self.d.train.get_size()):
            ...
            print ("\t%d/%d\tID = %d\tY = %d" % (i+1, self.d.train.get_size(), b['id'][0], b['y'][0]))
            start_time = time.time()
            tag, x, typ = self.attack(b['raw'][0], b['y'][0], self.inss['stmt_tr'][b['id'][0]], n_candidate, n_iter)
            if tag:
                n_succ += 1
                total_time += time.time() - start_time
                fail_pred_raws.append(x)
...
if __name__ == "__main__":
    ...
    #atk = AdversarialTrainingAttacker(poj, symtab, model, word2vec)
    #atk.attack_all(40, 50, res_save="../model/astnn/uid_rename_atk_candi40_iter50.advsamples.pkl.gz", adv_sample_size=5000)

    atk = AdversarialTrainingInsAttacker(poj, instab, model, word2vec)
    atk.attack_all(40, 20, res_save="../model/astnn/stmt_insert_atk_candi40_iter20.advsamples.pkl.gz", adv_sample_size=2000)