...
def get_batched_data(raws1, raws2, ys, txt2idx, max_stmt_cnt, max_stmt_len, vocab_size, ids1=None, ids2=None):

    xs1, ls1 = raw2x(raws1, txt2idx, max_stmt_cnt, max_stmt_len, vocab_size)
    xs2, ls2 = raw2x(raws2, txt2idx, max_stmt_cnt, max_stmt_len, vocab_size)

    batch = {"x1": [], "l1": [], "raw1": [], "id1": [], 
             "x2": [], "l2": [], "raw2": [], "id2": [],
             "y": [], "new_epoch": False}
    batch['x1'] = xs1
    batch['l1'] = ls1
...
class TokenModifier(object):
    ...
    def rename_uid(self, x_raw1, x_raw2, y, ori_uid, n_candidate=5):
        ...
        batch = get_batched_data([x_raw1], [x_raw2], [y], self.txt2idx, self.max_stmt_cnt, self.max_stmt_len, self.cl.vocab_size)
        inputs1, inputs2, lens1, lens2, labels= gettensor(batch, self.cl.device)
        grad = self.cl.grad(inputs1, inputs2, lens1, lens2, labels, self.loss)
        ...
        for new_uid in new_uid_cand:
            if not self.uids[new_uid]:
                continue
            if self.idx2txt[new_uid] in x_raw1:
                continue
            ...
            for i in range(len(new_x_raw1[-1])):
                if new_x_raw1[-1][i] == ori_uid_raw:
                    new_x_raw1[-1][i] = self.idx2txt[new_uid]
            try:
